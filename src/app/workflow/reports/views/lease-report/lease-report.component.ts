...
import { IArea, ICompany, IContract, ICounterPartBD, ICountry, ILocationType, ReporteArrendamiento } from '@speed/common/interfaces';
...
export class LeaseReportComponent implements OnInit, AfterViewInit {
  ...
  public data: ReporteArrendamiento[] = [];
  ...
  public openOtherTab(event: any) {
    const params = { id: event.idExpediente, code: event.code };
  ...
  public changeCountry(event: Event): void {
    ...
    this.cacheService.set('Reporte de Arrendamiento', this.cachedData);
  ...
  public changeCompany(event: Event): void {
    ...
    this.filterAreas = this.areasList.filter((area) => area.compania.id === selectedCompany);
    this.filterUbicacionesList = this.ubicacionesList.filter(
      (ubicacion) => ubicacion.tipo_ubicacion.id == this.requestForm.get('tUb')?.value && ubicacion.compania.id == selectedCompany,
    );
  ...
  public changeTipoUbicacion(event: Event): void {
    ...
    this.filterUbicacionesList = this.ubicacionesList.filter(
      (ubicacion) => ubicacion.tipo_ubicacion.id == selectedTipoUbicacion && ubicacion.compania.id == this.requestForm.get('idC')?.value,
    );
    this.cachedData.filterUbicacionesList = this.filterUbicacionesList;
    this.cacheService.set('Reporte de Arrendamiento', this.cachedData);
    if (selectedTipoUbicacion > 0) {
  ...
  public async submit() {
    ...
    if (this.requestForm.valid) {
      ...
      this.data = await this.reportService.getReporteArrendamiento(params);
  ...
  public exportReport(format: string) {
    ...
    if (this.requestForm.valid) {
      const url = String(
        environment.reporteArrendamientoSpeed +
        '&rs:Format=' +
        format +
  ...
  public onEditDocument(contract: IContract) {
    this.contractSelected = contract;
    this.tabsComponent.openTab(`N° ${contract.code === null ? '""' : contract.code}`, this.editContractTemplate, contract, true);
  }
  ...
  private async recoverData() {
    this.cachedData = this.cacheService.get('Reporte de Arrendamiento');
