...
@Component({
  selector: 'ui-automatic-addendum-form',
...
export class AutomaticAddendumFormComponent implements OnInit {
  ...
  public initTabs() {
    const documentType = this.addendumForm.get('tipoDocumento')?.value;
    this.tabApplicationDocuments = getTabApplicationDocuments(ProcessType.ADENDA_AUTOMATICA, documentType);
    ...
    this.requestForm.addControl('tipoDocumento', this.addendumForm.get('tipoDocumento'));

    // const control = new FormControl('tipoDocumento', Validators.required);
    //   ((this.requestForm.get('tipoDocumento') as FormGroup).get('files') as FormArray).push(control);
